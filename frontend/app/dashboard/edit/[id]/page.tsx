...
// Categories data (same as add page)
const CATEGORIES = [
  { id: 1, name: 'Fast Food' },
  { id: 2, name: 'Nigerian' },
  { id: 3, name: 'Chinese' },
...
export default function EditRestaurantPage() {
  ...
  useEffect(() => {
    if (!authLoading && user) {
      if (user.role !== 'vendor' && user.role !== 'admin') {
        toast.error('You do not have permission to edit restaurants');
        router.push('/');
      }
    }
  ...
  const loadRestaurant = async () => {
    ...
    try {
      ...
      if (restaurant.user_id !== user?.id && user?.role !== 'admin') {
      ...
      setFormData({
        ...
        categories: restaurant.categories?.map((cat: any) => cat.id) || [],
        image: null,
        existing_image: restaurant.image,
      });
      
      if (restaurant.image) {
        const imageUrl = `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}/storage/${restaurant.image}`;
  ...
  const removeImage = () => {
    setFormData(prev => ({ ...prev, image: null, existing_image: null }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateRestaurant(restaurantId, submitData);
  ...
  const hasChanges = (): boolean => {
    ...
    const hasBasicChanges = 
      ...
      formData.latitude !== originalData.latitude.toString() ||
      formData.longitude !== originalData.longitude.toString() ||
      formData.image !== null;
    
    const categoryChanged = JSON.stringify(formData.categories.sort()) !== 
      JSON.stringify(originalData.categories?.map((c: any) => c.id).sort() || []);
    
    return hasBasicChanges || categoryChanged;
  };
